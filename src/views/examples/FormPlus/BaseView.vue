...
<script lang="ts" setup>
  ...
  import type { IFormPlusProps, IFormPlusRef, IFormPlusSchema } from '@bep-ui/components'
  import type { Recordable } from '@bep-ui/types/common'
  import { states, sizeOptions, layoutOptions } from '@/const/select'

  interface ISetting extends Omit<IFormPlusProps, 'labelWidth'> {
    labelWidth: boolean
  }
  ...
  const settings = reactive<Partial<ISetting>>({
    labelWidth: false,
    size: 'default',
    layout: 'block'
  })

  const list: Recordable[] = states.map((item, index) => ({
    value: `${index}`,
    label: item
  }))

  const schema: IFormPlusSchema = {
    baseInfo: {
      title: '基本信息',
